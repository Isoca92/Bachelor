...
\vspace{0.5cm}
Diese Laufzeitanalyse ergibt eine annähernd lineare Regression. Der genutzte Algorithmus nach Ullmann benötigt im schlimmsten Fall, bei einem Anfrage-SMARTS mit einer Knotenanzahl $n$ und einem Ziel-SMARTS mit der Knotenanzahl $m$, eine Laufzeit, die in $\mathcal{O}(m^nn^2)$ liegt. Bei dem Aufbau der Relations-Matrix werden $nm$ Vergleiche und bei dem Aufbau der Kanten-Verträglichkeit $(n-1)(m-1)$ Vergleiche benötigt. Insgesamt lässt sich die asymptotische \textit{Worst-case} Laufzeit des entwickelten Verfahrens auf $\mathcal{O}(m^nn^2)$ abschätzen.
%
% EOF
%