...
Dem Programm werden neben dem Anfrage-SMARTS und Ziel-SMARTS zusätzlich die zu untersuchende Teilmengen-Relation übergeben.

\subsection{Genutze Software}
Das in dieser Arbeit vorgestellte Verfahren wurde im Rahmen der NAOMI-Softwarebibliothek \cite{naomi} des Zentrums für Bioinformatik (ZBH) der Universität Hamburg implementiert. Aus den folgende Modulen wurden Funktionen und Klassen genutzt:
\begin{enumerate}
...
\begin{table}[h]
	...
	\begin{tabular}{| l | l |}
		...
		Größe des kleinsten SSSR-Ring in dem & \texttt{r<n>} \\
		sich das Atom befindet & \\
...
\subsection{Vergleich und Aufbau Relations-Matrix} \label{ssec:relmatr}
...
Anschließend wird die Initial-Matrix der Knoten und die der Kanten zusammen mit den Adjazenzmatrizen\footnote[1]{Matrix zugehörig zu einen Graphen. Matrixeintrag "`1"' bedeuted, dass eine Kante zwischen zwei Knoten besteht. Matrixeintrag "`0"', dass es keine Kante zwischen zwei Knoten gibt. Dabei besitzt sie für jeden Knoten eine Zeile ($i$) und eine Spalte ($j$). Ein Element ($i,j$) gibt die Adjazenz vom $i$-ten und $j$-ten Knoten an. \cite{graph}} der beiden chemischen Mustern an einen Algorithmus zur Berechnung des Isomorphismus übergeben. In dieser Arbeit wurde dazu der Algorithmus nach Ullmann \cite{ullmann} verwendet.\\
...
Beispiel \ref{bsp:intersectwo} zeigt zwei SMARTS-Ausdrücke bei denen Formel \eqref{eq:fall2} gilt. Für mindestens einen Knoten gilt $\supset$. In diesem Beispiel ist es jeweils der letzte Knoten. Des Weiteren gilt für mindestens einen Knoten- oder Bindungspaar $\subset$. In diesem Beispiel sind dies die jeweils ersten beiden Knoten und die Bindung.\\
